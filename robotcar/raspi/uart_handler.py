...
class UARTHandler:
    def __init__(self, port="/dev/serial0", baudrate=1000000):
        ...
        self._command = {
            ...
            "enable_stability_control":   False,
            "mode":                       0,
            "move_speed":                 0.0,
            "acceleration":               0.0,
            "steer":                      0.0,
    ...
    def _build_packet(self) -> bytes:
        ...
        if cmd["enable_traction_control"]:   flags |= 0x40
        if cmd["enable_stability_control"]:  flags |= 0x80

        mode         = int(cmd["mode"]) & 0xFF
        ...
        return struct.pack("BBBbbbB", HEADER, flags, mode, move_speed, acceleration, steer, FOOTER)
    ...
    def _parse_packet(self, pkt: bytes):
        ...
        motor_slip = bool(pkt[1] & 0x01)
        motor_err  = bool(pkt[1] & 0x02)
