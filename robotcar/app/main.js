...
const dispThr     = document.getElementById("disp-thr");
const dispUptime  = document.getElementById("disp-uptime");
...
function connect() {
  ...
  ws.onmessage = (e) => {
    ...
    const slip = d.motor_slip;
    dispErr.textContent = err ? (slip ? "ERR+SLIP" : "ERROR") : (slip ? "SLIP" : "OK");
    dispErr.className   = "hdr-telem-val" + (err ? " error" : slip ? " warn" : "");
    ...
    if (thr == null) {
      dispThr.textContent = "--";
      dispThr.className = "hdr-telem-val";
    } else {
      const cur = thr & 0xF;
      dispThr.textContent = cur === 0 ? "OK" : (thr & 0x1 ? "UV" : "THRTL");
      dispThr.className = "hdr-telem-val" + (cur === 0 ? "" : (thr & 0x1) ? " error" : " warn");
    }

    dispUptime.textContent = d.uptime ?? "--";
  };
}
...
const btnTraction = document.getElementById("btn-traction");
btnTraction.addEventListener("click", () => {
  state.enable_traction_control = !state.enable_traction_control;
  btnTraction.classList.toggle("active", state.enable_traction_control);
  sendState();
});

const btnStability = document.getElementById("btn-stability");
btnStability.addEventListener("click", () => {
  state.enable_stability_control = !state.enable_stability_control;
  btnStability.classList.toggle("active", state.enable_stability_control);
  sendState();
});
...
speedVal.textContent = `${(targetSpeedRaw * 0.1).toFixed(1)} m/s`;
accelVal.textContent = `${(targetAccelRaw * 0.1).toFixed(1)} m/s²`;
btnAutobrake.classList.add("active");
btnTraction.classList.add("active");
btnStability.classList.add("active");
initRadar();
connect();

// Heartbeat: 500ms ごとに現在の状態を送信し、サーバー側の切断検知を可能にする
