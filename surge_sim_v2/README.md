...
- **SLAMLocalizer**: Hector風 Gauss-Newton スキャンマッチ。占有格子の確率場に
  スキャン終点を合わせ込む。マルチ解像度（粗→細のボカし、σはメートル指定で解像度非依存）。
- **IMUヨー融合**: 実機IMU（AHRSヨー）を方位の事前分布中心に使い、スキャンマッチは
  x,y に集中させる。方位ドリフトが消え、最も効く改善。シミュは IMU を模擬（ノイズ＋有界バイアス）。
- **運動モデル事前分布**: 等速度予測（SLAM軌跡由来、オドメトリではない）をソフト拘束
  として GN に組み込み、直線で拘束不足になる縦方向（アパーチャ問題）を補う。
- **増分マッピング**: 一度確定したセルは凍結し未知領域だけ埋める＋キーフレーム間引きで、
  推定の微小誤差が地図に焼き込まれて発散するのを防ぐ。
- **半セル整合**: 占有はセル中心にあるので補間インデックスを半セルずらす（バイアス除去）。
- **占有格子 3cm**（5cm から高解像度化）。

検証（1周通し・両コース、IMUあり）: 自己位置推定誤差 **平均約3cm**、方位 **0.8°**、
多周回でも誤差が育たない（ループクロージャ不要）、自律走行で衝突なし。
IMU を切ると（`imu.enabled: false`）直線で縦ドリフトが増える。
...
## コース
`maps/*.py` を自動スキャン。同梱コース：
- **default_course**（長方形リング、幅1m）
- **course_a**（L字、直角コーナー1箇所）
- **course_b**（Complex Circuit：シケイン＋スイーパーの流れる周回路）
- **course_c**（Grand Circuit：ロングストレート＋スイーパー＋シケイン＋エセスの大型周回路）
  course_b/c は中心線を平滑化して左右に半幅オフセットし壁を自動生成（`maps/_track.build_loop`）。

## 動的障害物・回避
- シミュに動的障害物（人・車＝円）を置ける（静止/移動）。LiDAR がレイ–円交差で検知。
- 自律走行中、**SLAM占有地図で「地図上は free なのに点が返る＝静的地図に無い物体」を障害物と判定**
  （壁は occupied なので誤検知しない）。経路前方の障害物に対し、至近は緊急停止、検知距離内は
  空いた側へ操舵＋減速で回避。`config/sim.yaml` の `obstacle` 節で調整。
- WebUI から `POST /api/sim/obstacle`（前方に出現）/ `POST /api/sim/obstacle/clear`。pygame はマゼンタ円で表示。

## 遅延（実機相当）
- `simulation.command_latency_ms`（既定40ms）でアクチュエータ遅延を模擬（UART往復＋STM32＋サーボ応答）。
  実機の遅延は制御安定性に直結するため、これを入れて制御の頑健性を確認できる。

## LiDAR モデル（LD06 データシート準拠）
`config/sim.yaml` の `simulation.lidar`：
- 測距範囲 0.02〜12m、角度分解能 既定1°/360点（実機LD06は約0.8°/450点、`n_rays`で変更可）
- **距離依存ノイズ** σ(d)=max(noise_floor, noise_pct×d)（≒±1%クラス）。
  既定で σ=1cm@1m、3.6cm@6m、7.2cm@12m。近距離は正確、遠距離ほどばらつく実機特性を再現。
