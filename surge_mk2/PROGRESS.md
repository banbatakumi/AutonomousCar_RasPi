...
**picamera2 の `format="RGB888"` は、numpy 配列上では BGR 順になる**という
既知の癖がある（libcamera の命名がリトルエンディアンのパック順を指すため）。
今回の撮影画像は不自然さが無かったが、**赤いものを撮って青く写らないかを確認するまで
確定ではない**。配管はバイトを忠実に運んでいるので影響は解釈だけ。
`FrameDesc.fmt` に文字列は載せてあるので、判明したら下流の解釈を合わせればよい。
