...
**追記（長距離ドリフト対策、`core/confidence.py`の情報行列推定を作り直した後）**:
上記の推測は当たっていた。`core/confidence.py`の旧手法（`grid.score_map()`
の有限差分＝局所曲率ベース）は、探索を安定させるために意図的に壁の周りへ
なだらかに盛った尤度場を微分していたため、差分刻み幅をどれだけ細かくしても
実際の到達精度（実測でオーバル直線区間、独立ノイズ40回試行の位置ばらつき
0.5cm程度）を約40〜50倍も過小評価していた（同条件で旧手法は約22cm相当の
不確かさを示唆）。点対応ベースのFisher情報（点対平面＋点対点の混合、
`core/confidence.py`の`_estimate_information_analytic`）に置き換え、かつ
「毎周期のライブブレンド」と「キーフレームに添えるエッジ重み」を別々に
算出するよう`core/frontend.py`を分離した（新手法を両方に使うと、ライブ
ブレンドの方は`info_pred`に対して`info_obs`が過大になり毎周期ほぼ生の
`obs_pose`に張り付いて平滑化が効かなくなり、oval3周のヨー誤差が0.85°から
14〜65°へ悪化する退行を実測したため。ライブブレンドは旧手法のまま、
キーフレーム＝エッジ重みだけ新手法にした）。

結果、`tests/test_pipeline_oval.py`の基準ベンチマーク（oval3周、seed=7）で
**史上初めてループ閉じがFrontend単体を上回った**（flush後の最終ヨー誤差
0.46° vs Frontend単体0.85°。旧手法では常に1.3°前後でFrontend単体に届か
なかった）。ただし複雑コース（`tests/test_pipeline_complex_course.py`の
`circuit_chicane_*`）では3例中2例で改善・1例で悪化という結果に留まって
おり、一貫した改善には至っていない——`min_index_gap`・`radius`・
`yaw_tolerance`等のパラメータ調整（次点の改善候補）が必要と見られる。

**現状の結論**: このモジュール（`backend/`一式、`pipeline.SlamSystem`の
バッチ最適化込み）は「複数の拘束を扱える枠組み」として動作し、情報行列の
校正精度を上げたことで単純な周回コースでは初めてFrontend単体を上回った。
複雑なコースではまだ一貫した改善に至っておらず、パラメータチューニングの
余地が残る。
"""
