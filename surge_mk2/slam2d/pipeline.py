...
それでも当初は`Frontend`単体（`flush()`後で約0.85°）にはまだ届いておらず、
コースや乱数シードによっては届かないことの方が多かった（オドメトリの蓄積
誤差が既に小さく、広域探索1回のノイズの方が相対的に大きいため）。

**追記**: `core/confidence.py`の情報行列推定を局所曲率ベースから点対応
Fisher情報ベースに作り直した後、この基準ベンチマーク（oval3周、seed=7）で
**史上初めてFrontend単体を上回った**（flush後0.46° vs Frontend単体0.85°）。
詳細と複雑コースでの残課題は`backend/loop_detection.py`のモジュール
docstring「追記」参照。
"""
