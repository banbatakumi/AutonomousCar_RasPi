...
| **ステアリングモータ ×1** | BLDC + 自作 MD。**位置モード**（サーボではない） | モータ角 → 路面舵角の**リンク比が未実測** |
...
> ⚠ **`measured = false`。** ホイールベース・車輪半径・リンク比は**未実測の暫定値**で、
> 「舵を何 rad 切ったか」「何 m 進んだか」が正しく分からない状態にある
> （`architecture.md` §15 の #2〜#4）。**Phase 4 の経路追従までには必要。**
...
**物理が浅くセンサが作り込んである**のは意図的。車両諸元は未実測で**合わせ込む相手がまだ無い**
一方、綺麗すぎる点群で開発すると**実車で初めて破綻する**ため。
...
| **ステアリングのリンク比・車輪半径が未実測** | **クリティカルパス。** これが無いと経路追従が成立しない |
