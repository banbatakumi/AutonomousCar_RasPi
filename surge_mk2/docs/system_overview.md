...
を新規に追加した。単体テストは通っているが、**シム・実車での走行検証はまだ無い。**
GUI・シムでは選べるが、採否は `sim.bench` の数値で決めること（development.md §4.3）。
...
点群パターンに対する挙動は原理的に保証できない。以前はそこを`stop_dist`（正面の
余裕が閾値を切ったらモデル出力を無視して無条件停止）という独立した安全策で受けて
いたが、STM32の`auto_stop`（速度に応じて伸びる動的停止距離。§9.3）の方が高性能
なため2026-09-12に撤去した。学習・ONNX化・GUIでの使い方は
[`development.md` §12](development.md#12-学習ベースの自動運転を動かすlidar-e2e--カメラセグメンテーション)。
...
> 倒している判断で、「開けた場所で一歩も動けなくなるくらいなら、STM32の`auto_stop`
> （★v0.12・速度に応じて伸びる動的停止距離 `d_stop`、LiDAR主・超音波補助）で受ける」
> という取引の上に成り立っている。**planner側は同種の固定距離しきい値をもう持たない**
> （2026-09-12、STM32側の方が高性能なため撤去した）。
...
| 「経路から落ちた」の検出 | 無い。STM32 の `auto_stop` が最後の砦 |
