...
| [uart_protocol.md](uart_protocol.md) | **UART プロトコル仕様**（**v0.13**）。仕様の説明の**正** | STM32 / RasPi 双方の実装者 |
...
- **プロトコルは v0.13**（`protocol.toml` の `protocol_version = 0x000D` が正）。
  **実機での動作確認は v0.9 まで**（2026-08-20）。v0.10〜v0.13 は Pi・STM32 とも
  実装済みだが実機での動作検証は未了
